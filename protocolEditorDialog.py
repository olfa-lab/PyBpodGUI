...
class SimpleItem(QGraphicsItem):
    ...
    def boundingRect(self):
        penWidth = 1.0
        return QRectF(self.x, self.y, 100 + penWidth, 225 + penWidth)

    def paint(self, painter, option, widget):
        painter.drawRoundedRect(0 + self.x, 0 + self.y, 100, 225, 5, 5)
        painter.drawText(0 + self.x, 0 + self.y, 100, 25, Qt.TextWordWrap | Qt.AlignCenter, f'State {self.num}')
        painter.drawLine(0 + self.x, 25 + self.y, 100 + self.x, 25 + self.y)
        painter.drawText(2 + self.x, 25 + self.y, 100, 25, Qt.TextWordWrap, f'name: {self.name}')
        painter.drawLine(0 + self.x, 50 + self.y, 100 + self.x, 50 + self.y)
        painter.drawText(2 + self.x, 50 + self.y, 100, 25, Qt.TextWordWrap, f'timer: {self.timer}')
        painter.drawLine(0 + self.x, 75 + self.y, 100 + self.x, 75 + self.y)
        painter.drawText(2 + self.x, 75 + self.y, 100, 75, Qt.TextWordWrap, f'change conditions: \n  {self.changeConditions}')
        painter.drawLine(0 + self.x, 150 + self.y, 100 + self.x, 150 + self.y)
        painter.drawText(2 + self.x, 150 + self.y, 100, 75, Qt.TextWordWrap, f'output actions: \n  {self.outputActions}')
...
class ProtocolEditorDialog(QDialog, Ui_Dialog):
    ...
    def __init__(self, parent=None):
        ...
        self.stateChangeNameComboBox.addItems(['Select...', 'exit'])
        self.outputChannelNameComboBox.addItems(self.outputChannels)

        self.scene = QGraphicsScene(parent=self)
        # self.scene = QGraphicsScene(0, 0, 501, 501, parent=self)  # The x, y values here override the x, y of the boundingRect in the SimpleItem class.
        # self.scene.addText("Hello, world!")
        # pen = QPen(Qt.green, 2, Qt.SolidLine)
        # brush = QBrush(Qt.green)
        # self.shape = self.scene.addRect(20, 40, 25, 100, pen=pen, brush=brush)
        # self.simp = SimpleItem(x=0, y=0, num=0, name='WaitForResponse', timer='10', change_conditions={'Tup': 'exit'}, output_actions={'Valve2': '1'})
        # self.scene.addItem(self.simp)
        # self.simp2 = SimpleItem(x=0, y=250, num=1, name='Reward', timer='2', change_conditions={'Tup': 'exit'}, output_actions={'Valve3': '1'})
        # self.scene.addItem(self.simp2)
    ...
    def connectSignalsSlots(self):
        ...
        self.stateTimerLineEdit.editingFinished.connect(self.recordStateTimer)
    ...
    def recordStateTimer(self):
        self.stateDict['stateTimer'] = int(self.stateTimerLineEdit.text())
    ...
    def recordOutputChannelName(self, channelName):
        ...
        # Note that 'LED' can be used multiple times as an output channel name (to turn on or off multiple LEDs within a single state).
        # So uncommenting this will require changing the stateDict because I made it a dict to avoid duplicates (since each key must be unique).
        # Will also require changing the ListWidget to show the multiple usages of the 'LED' output channel name.
        # elif (channelName == 'LED'):  # Need to add 'LED' to the list of outputChannels.
        #     self.outputChannelValueComboBox.clear()
        #     self.outputChannelValueComboBox.addItems(['Channel number...', '1', '2', '3', '4'])

        # Same issue here as with 'LED' above.
        # elif (channelName == 'Valve'):  # Need to add 'Valve' to the list of outputChannels. Also, make sure to check for this before startswith('Valve').
        #     self.outputChannelValueComboBox.clear()
        #     self.outputChannelValueComboBox.addItems(['Channel number...', '1', '2', '3', '4'])
        
        elif channelName.startswith('Serial') or (channelName == 'SoftCode'):
            self.outputChannelValueComboBox.clear()
