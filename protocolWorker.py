...
class ProtocolWorker(QObject):
    ...
    def __init__(self,
        ...
        self.stimConfigFileName = stimConfigFileName
        self.experimentType = experimentType  # this is text
        self.camera = camera
        ...
        self.vialIndex = 0  # more accurately called 'trialIndex'
        self.flowIndex = 0
        self.stimIndex = 0
        self.stimList = []
        self.nOlfas = 0
        ...
        if self.experimentType == '1PImaging': # We use this list only for imaging
            self.allTrialsDict = odorEditorObject.all_trials_dict  # list of dictionaries generated by odorEditorDialog  all_trials_dict
            self.nTrials = min(numTrials,len(allTrialsDict))
        elif self.experimentType == 'Auditory' or self.experimentType == 'Mixed' :
            self.soundStimDict = soundEditorObject.stimDict
    ...
    def my_softcode_handler(self, softcode):
        ...
        if softcode == 1:
        ...
        elif softcode == 2: # Reserved for OLFA - do NOT use for other experiments
            try:
            ...
            # When I have to prepare the function generator to deliver auditory stimulus
            # Set stim channel
            freq = self.stimList[0]['soundfreq']
            amp = self.stimList[0]['soundamp']
            duration = self.stimList[0]['soundtime']
            self.wg.disable_output_channel(self.wg_stim_channel)
            self.wg.set_waveform_freq(self.wg_stim_channel, freq)
            self.wg.set_waveform_amp(self.wg_stim_channel, amp)
            self.wg.enable_burst(self.wg_stim_channel)
            self.wg.set_burst_mode(self.wg_stim_channel, self.wg_stim_mode)
            self.wg.set_burst_duration(self.wg_stim_channel, duration)
            self.wg.set_trigger_source(self.wg_stim_channel, self.wg_trig_source)
            self.wg.wait_for_external_trigger(self.wg_stim_channel)


        # Update trial info for GUI
        #print(self.sma.state_names)
        #print(self.sma.current_state)
        ...
        if self.currentStateName == 'Correct':
            ...
            #print('checking that I can actually plot stuff')
            #print(self.experimentType, self.currentstimulusType)
            #print(( self.experimentType == 'Mixed' and self.currentstimulusType == 'O'))
            if (self.experimentType == 'Intensity' or self.experimentType == '1PImaging'):
                ...
                self.resultsCounterList[0][freq][amp]['Correct'] += 1
                self.resultsCounterList[0][freq][amp]['Total'] += 1
                self.resultsCounterListSignal.emit(self.resultsCounterList)
            elif self.experimentType == 'Mixed':
                if self.currentstimulusType == 'S':
                    stimtype = self.stimList[0]['soundfreq']
                    stimamp = "{:.2f}".format(self.stimList[0]['soundamp'])
                    self.resultsCounterList[1][stimtype][stimamp][self.correctResponse] += 1
                    self.resultsCounterList[1][stimtype][stimamp]['Correct'] += 1
                    self.resultsCounterList[1][stimtype][stimamp]['Total'] += 1
                    self.resultsCounterListSignal.emit(self.resultsCounterList)
                elif self.currentstimulusType == 'O':
                    vialNum = self.stimList[0]['olfas']['olfa_0']['vialNum']
                    flow = self.stimList[0]['olfas']['olfa_0']['mfc_1_flow']
                    self.resultsCounterList[0][vialNum][str(flow)][self.correctResponse] += 1
                    self.resultsCounterList[0][vialNum][str(flow)]['Correct'] += 1
                    self.resultsCounterList[0][vialNum][str(flow)]['Total'] += 1
                    self.resultsCounterListSignal.emit(self.resultsCounterList)

            elif (self.experimentType == 'Identity'):
                for i in range(self.nOlfas):
            ...
            if (self.experimentType == 'Intensity' or self.experimentType == '1PImaging'):
                ...
                amp = "{:.2f}".format(self.stimList[0]['soundamp'])
                if self.correctResponse == 'left':
                    self.resultsCounterList[0][freq][amp]['right'] += 1  # increment the opposite direction because the response was wrong.
                elif self.correctResponse == 'right':
                    self.resultsCounterList[0][freq][amp]['left'] += 1  # increment the opposite direction because the response was wrong.
                self.resultsCounterList[0][freq][amp]['Wrong'] += 1
                self.resultsCounterList[0][freq][amp]['Total'] += 1
                self.resultsCounterListSignal.emit(self.resultsCounterList)
            elif self.experimentType == 'Mixed':
                if self.currentstimulusType == 'O':
                    ...
                    flow = self.stimList[0]['olfas']['olfa_0']['mfc_1_flow']
                    if self.correctResponse == 'left':
                        self.resultsCounterList[0][vialNum][str(flow)]['right'] += 1  # increment the opposite direction because the response was wrong.
                    elif self.correctResponse == 'right':
                        self.resultsCounterList[0][vialNum][str(flow)]['left'] += 1  # increment the opposite direction because the response was wrong.
                    self.resultsCounterList[0][vialNum][str(flow)]['Wrong'] += 1
                    self.resultsCounterList[0][vialNum][str(flow)]['Total'] += 1
                    self.resultsCounterListSignal.emit(self.resultsCounterList)
                    ...
                    amp = "{:.2f}".format(self.stimList[0]['soundamp'])
                    if self.correctResponse == 'left':
                        self.resultsCounterList[1][freq][amp]['right'] += 1  # increment the opposite direction because the response was wrong.
                    elif self.correctResponse == 'right':
                        self.resultsCounterList[1][freq][amp]['left'] += 1  # increment the opposite direction because the response was wrong.
                    self.resultsCounterList[1][freq][amp]['Wrong'] += 1
                    self.resultsCounterList[1][freq][amp]['Total'] += 1
                    self.resultsCounterListSignal.emit(self.resultsCounterList)
            ...
            if (self.experimentType == 'Intensity'):
                ...
                freq = self.stimList[0]['soundfreq']
                amp = "{:.2f}".format(self.stimList[0]['soundamp'])
                self.resultsCounterList[0][freq][amp]['NoResponse'] += 1
                self.resultsCounterList[0][freq][amp]['Total'] += 1
                self.resultsCounterListSignal.emit(self.resultsCounterList)
                ...
                if self.currentstimulusType == 'O':
                    vialNum = self.stimList[0]['olfas']['olfa_0']['vialNum']
                    flow = self.stimList[0]['olfas']['olfa_0']['mfc_1_flow']
                    self.resultsCounterList[0][vialNum][str(flow)]['NoResponse'] += 1
                    self.resultsCounterList[0][vialNum][str(flow)]['Total'] += 1
                    self.resultsCounterListSignal.emit(self.resultsCounterList)
                elif self.currentstimulusType == 'S':
                    freq = self.stimList[0]['soundfreq']
                    amp = "{:.2f}".format(self.stimList[0]['soundamp'])
                    self.resultsCounterList[1][freq][amp]['NoResponse'] += 1
                    self.resultsCounterList[1][freq][amp]['Total'] += 1
                    self.resultsCounterListSignal.emit(self.resultsCounterList)
    ...
    def getOdorsFromConfigFile(self):
        ...
        print(' ------------- I am in getOdorsFromConfigFile -----------------------')
        ...
        if (self.experimentType == 'Intensity' or self.experimentType == 'Mixed' ):
    ...
    def createsoundIntensityStimuliDistribution(self):
        
        ##This all function is making the file useless
        self.all_freq_list = []
        self.all_amp_list = []
        self.all_dur_list = []
        ...
        for freq_key in self.soundStimDict.keys():
           
            for ii in range(len(self.soundStimDict[freq_key]['prob'])):
                stimprob =  int(self.soundStimDict[freq_key]['prob'][ii])
                stimamp =  self.soundStimDict[freq_key]['amps'][ii]
                stimdur=  self.soundStimDict[freq_key]['duration'][ii]
                self.all_freq_list.extend([freq_key]*stimprob)
                self.all_amp_list.extend([float(stimamp)]*stimprob)
                self.all_dur_list.extend([float(stimdur)]*stimprob)
        self.amp_TH = dict()
        for freq_key in self.soundStimDict.keys():
        ...
        # res1 and res2 come out as tuples, and so must be converted to lists.
        self.all_freq_list, self.all_amp_list , self.all_dur_list= list(res1), list(res2), list(res3) 
    
    
    def auditoryGenerator(self):

        self.createsoundIntensityStimuliDistribution()
        ostim = {}
        
        if (self.shuffleMultiplier > 0):
            currentFreq = self.all_freq_list[self.currentStimulusIdx]
            currentAmp = self.all_amp_list[self.currentStimulusIdx]
            currentDur = self.all_dur_list[self.currentStimulusIdx]
            amp_threshold = self.amp_TH[currentFreq]
            self.currentStimulusIdx+=1
            if self.currentStimulusIdx > len(self.all_freq_list)-2:
                self.currentStimulusIdx = 0
        # If flow is lower than flow_threshold == ~30 , 'left' is correct. Otherwise, 'right' is correct
        if (currentAmp < amp_threshold):
            self.correctResponse = 'left'
        else:
            self.correctResponse = 'right'       

        ostim = {
            'soundfreq': currentFreq,
            'soundamp': currentAmp,
            'soundtime': currentDur,
        }
        self.stimList.clear()
        self.stimList.append(ostim)
    ...
    def initializeSoundResponseDict(self):
        resultsCounterDict= dict()
        for freq_key in self.soundStimDict.keys():
            resultsCounterDict[freq_key] = dict()
            for amp_key in self.soundStimDict[freq_key]['amps']:
                AMP_key = "{:.2f}".format(float(amp_key))
                resultsCounterDict[freq_key][AMP_key] = {
                                    'right': 0,  # initizialize counters to zero.
                                    'left': 0,
                                    'Correct': 0,
                                    'Wrong': 0,
        ...
        match self.experimentType:
            ...
            case '1PImaging':
                ...
                # Note that these except clauses can only trigger from the first trial.
                    ...
                    self.stopRunning()  # This would also stop the bpod trial just in case the olfa raised the exception when the state machine is running.
                    # self.finished.emit()
                # Setting stimulus function to Imaging Generatior    
                self.stimulusFunction = self.imagingGenerator
            case 'Auditory': 
    ...
    def startTrial(self):
        if self.keepRunning and (self.currentTrialNum <= self.nTrials) and (self.consecutiveNoResponses < self.noResponseCutOff):
            ...
            if self.experimentType== 'Mixed':
                ...
                if self.currentstimulusType == 'S':
                    self.protocolFileName = self.prepend_S(self.protocolFileName)
                elif  self.currentstimulusType == 'O':
                    self.protocolFileName = self.prepend_O(self.protocolFileName)
            ...
            for state in self.stateMachine['states']:
                if 'Olfactometer' in state['outputActions']:
                    ...
                    # Replace 'Olfactometer': 'set_dummy_vials' with SoftCode 3.
                        ...
                        del state['outputActions']['Olfactometer']  # Delete 'Olfactometer because its not a valid output channel.
                if 'Speaker' in state['outputActions']:
                    # Replace 'Olfactometer': 'set_stimulus' with SoftCode 4
                    if (state['outputActions']['Speaker'] == 'set_stimulus'):
                        state['outputActions']['SoftCode'] = 4  # SoftCode 4 is reserved for set_stimulus.
                        del state['outputActions']['Speaker']  # Delete 'Speaker' because its not a valid output channel.
            ...
            try:
                ...
                print('error in running the state machine')
